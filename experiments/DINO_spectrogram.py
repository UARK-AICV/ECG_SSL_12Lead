...
from utils.DINO_dataloader import ECG_dataset_DINO_spectrogram
...
def run():
    ...
    train_dataset = ECG_dataset_DINO_spectrogram(summary_folder=data_folder, signal_size=signal_size, stride=train_stride,
                                                chunk_length=train_chunk_length,equivalent_classes=equivalent_classes, 
                                                sample_items_per_record=1, random_crop=True)
    train_dataloader = DataLoader(train_dataset, shuffle=True, num_workers=8,batch_size=batch_size,drop_last=True)

    no_classes = 24
    student = spectrogram_model(no_classes)
    teacher = spectrogram_model(no_classes)
    ...
    for epoch in range(1,no_epoches+1):
        ...
        if whole_train_loss < lowest_train_loss:
            ...
            torch.save(student.backbone.state_dict(), f'./checkpoints/DINO_spectrogram_student.pth')
            torch.save(teacher.backbone.state_dict(), f'./checkpoints/DINO_spectrogram_teacher.pth')
