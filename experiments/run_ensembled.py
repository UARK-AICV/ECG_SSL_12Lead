...
def run():
    ...
    for no_fold in fold_range:
        ...
        train_dataset = ECG_dataset_base(summary_folder=data_folder, classes=classes, signal_size=signal_size, stride=train_stride,
                                    chunk_length=train_chunk_length, transforms=transforms, stft_inc=True, meta_inc=False, t_or_v='train',
        ...
        if args.finetune is not None:
            ...
            w_time = os.path.join(checkpoint_folder,"DINO_signal_student.pth")
            w_spec = os.path.join(checkpoint_folder,"DINO_spectrogram_student.pth")
            model = ensemble_model(no_classes, args.gating,w_time,w_spec,ctx)
        else:
            model = ensemble_model(no_classes, args.gating)
        ...
        for epoch in range(1,no_epoches+1):
            ...
            for batch_idx, sample in enumerate(tqdm(train_dataloader)):
                ...
                stft = stft.view(-1,no_channels,13,21)
            ...
            with torch.no_grad():
                ...
                if auroc > best_auroc:
                    best_auroc = auroc
                    if args.gating:
                        torch.save(model.state_dict(), f'./checkpoints/{args.save_folder}/{args.model_type}_withgating_fold{no_fold}_bestROC.pth')
                    else:
                        torch.save(model.state_dict(), f'./checkpoints/{args.save_folder}/{args.model_type}_wthoutgating_fold{no_fold}_bestROC.pth')
                
                if auprc > best_auprc:
                    best_auprc = auprc
                    if args.gating:
                        torch.save(model.state_dict(), f'./checkpoints/{args.save_folder}/{args.model_type}_withgating_fold{no_fold}_bestPRC.pth')
                    else:
                        torch.save(model.state_dict(), f'./checkpoints/{args.save_folder}/{args.model_type}_wthoutgating_fold{no_fold}_bestPRC.pth')
