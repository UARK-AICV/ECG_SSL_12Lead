...
class TGaussianNoise(Transformation):
    ...
    def __init__(self, scale=[0.005,0.01]):
    ...
    def __call__(self, sample):
        if self.scale ==0:
            ...
            data, label = sample
            scale_ratio = random.uniform(*self.scale)
            data = data + scale_ratio * torch.randn(data.shape)
            return data, label
...
class TimeOut_difflead(Transformation):
    """ replace random crop by zeros differently on each lead
    """

    def __init__(self, crop_ratio_range=[0.0, 0.5]):
        super(TimeOut_difflead, self).__init__(crop_ratio_range=crop_ratio_range)
        self.crop_ratio_range = crop_ratio_range
    ...
    def __call__(self, sample):
        ...
        data = data.copy()
        timesteps, channels = data.shape
        crop_ratios = [random.uniform(*self.crop_ratio_range) for i in range(channels)]
        ...
        for cha in range(channels):
            data[start_idxes[cha]:start_idxes[cha]+crop_timesteps[cha],cha] = 0

        return data, label

    def __str__(self):
        return "TimeOut_difflead"

class TChannelReduction(Transformation):
    """Remove a few leads"""
    
    def __init__(self, reduction_size=(1, 3)):
        super(TChannelReduction, self).__init__()
        self.reduction_size = reduction_size
        
    def __call__(self, sample):
        data, label = sample
        no_reduction = random.randint(*self.reduction_size)
        reduction_channels = np.random.choice(np.arange(0,12),no_reduction,replace=False) 
        for cha in reduction_channels:
            data[:,cha] = data[:,cha]*0

        return data, label
    
    def __str__(self):
        return "ChannelReduction"
    
class TTimeOut_difflead(Transformation):
    """ replace random crop by zeros differently on each lead
    """

    def __init__(self, crop_ratio_range=[0.0, 0.5]):
        super(TTimeOut_difflead, self).__init__(crop_ratio_range=crop_ratio_range)
        self.crop_ratio_range = crop_ratio_range
...
class TTimeShift(Transformation):
    """ replace random crop by zeros differently on each lead
    """

    def __init__(self, shift_range=[0.0, 0.5]):
        super(TTimeShift, self).__init__(shift_range=shift_range)
    ...
    def __call__(self, sample):
        ...
        data = data.clone()
        timesteps, channels = data.shape
        shift_ratios = [random.uniform(*self.shift_range) for i in range(channels)]
        shift_timesteps =  [int(sh*timesteps) for sh in shift_ratios]
        zero_arr = torch.zeros(timesteps,channels)
        for cha in range(channels):
            zero_arr[0:shift_timesteps[cha],cha] = data[timesteps - shift_timesteps[cha]-1:timesteps-1,cha]
            zero_arr[shift_timesteps[cha]:timesteps,cha] = data[0:timesteps - shift_timesteps[cha],cha]
...
class SpectralAug_freqonly(Transformation):
    # with nyq fred = 1/2 fs, [0,26] [0hz,50hz] with skip of 2hz
    def __init__(self, freq_range=[0,16], freq_band=[1,4]):
        super(SpectralAug_freqonly, self).__init__()
        self.freq_range = freq_range
        self.freq_band = freq_band

    def __call__(self, sample):
        stft_data, label = sample
        band_length = random.randint(*self.freq_band)
        start_freq = random.randint(self.freq_range[0],self.freq_range[1]-band_length)
        signal_data = []
        for cha in range(12): # 12 channels
            stft_data[cha][start_freq:start_freq+band_length,:] = 0
            _, x_rec = istft(stft_data[cha], 100)  # fs = 100
            signal_data.append(x_rec)
        signal_data = np.array(signal_data)
        signal_data = np.transpose(signal_data,(1,0))

        return signal_data, label

    def __str__(self):
        return "SpetralAug_freqonly"
